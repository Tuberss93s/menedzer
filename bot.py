...
class UltimateKore(discord.Client):
    ...
    async def status_rotator(self):
        ...
        while not self.is_closed():
            msgs = ["⚙️ ᴅᴇᴠᴇʟᴏᴘᴇʀ @ᴋɪᴛsᴜɴᴇ_𝟸𝟻𝟸𝟶ᴢᴀᴘᴀs", "✅ ᴄᴢᴇᴋᴀɴɪᴇ ɴᴀ ᴘᴀʀᴛɴᴇʀsᴛᴡᴀ", "🛠️ ᴍᴀᴅᴇ ɪɴ ᴋᴏʀᴇ sʜ𝟶ᴘ", "💎 discord.gg/9jUSJcT2PF", f"🤝 ᴏsᴛᴀᴛɴɪᴇ: {self.ostatni_serwer}", f"📊 ᴘᴀʀᴛɴᴇʀsᴛᴡ: {len(self.uzyte_serwery)}", "📦 ɴᴏᴡᴇ ᴅᴏsᴛᴀᴡʏ ᴡʟᴇᴄɪᴀᴌʏ!", "💳 ᴘᴌᴀᴛɴᴏsᴄɪ: ᴛʏʟᴋᴏ ᴘsᴄ 🎫", "🛡️ 𝟷𝟶𝟶% ʟᴇɢɪᴛ & ᴠᴇʀɪғɪᴇᴅ", "🎫 ᴏᴛᴡᴏ́ʀᴢ ᴛɪᴄᴋᴇᴛ ʙʏ ᴋᴜᴘɪᴄ́", "🚀 ɴᴀᴊsᴢʏʙsᴢᴀ ʀᴇᴀʟɪᴢᴀᴄᴊᴀ"]
            for m in msgs:
                try:
        ...
        if message.author.id == ADMIN_ID:
            if message.content == "!help":
                help_txt = (
                    ...
                    "┕ `!usun [ɪᴅ_sᴇʀᴡᴇʀᴀ] [ᴘᴏᴡᴏ́ᴅ]` - ᴜsᴜᴡᴀ ᴘᴀʀᴛɴᴇʀsᴛᴡᴏ ɪ ʙʟᴏᴋᴜᴊᴇ\n"
                    "┕ `!potwierdz [ɪᴅ_ᴜᴢʏᴛᴋᴏᴡɴɪᴋᴀ]` - ᴏᴅʙʟᴏᴋᴏᴡᴜᴊᴇ ᴜᴢʏᴛᴋᴏᴡɴɪᴋᴀ"
                )
                await message.channel.send(help_txt); return

            if message.content == "!status":
                await message.channel.send(f"📊 **sᴛᴀᴛʏsᴛʏᴋɪ ᴋᴏʀᴇ sʜ𝟶ᴘ**\n┕ ᴘᴀʀᴛɴᴇʀsᴛᴡᴀ: `{len(self.uzyte_serwery)}` \n┕ ʙʟᴏᴋᴀᴅʏ: `{len(self.zablokowani_uzytkownicy)}` osób"); return
            ...
            if message.content.startswith("!usun"):
                ...
                if srv_id in self.uzyte_serwery:
                    ...
                    if ids[1]: # Usuwanie z kanału partnerstw
                        try:
                            m_p = await part_ch.fetch_message(ids[1]); await m_p.delete()
                        except: pass
                    if ids[0] != "EXISTING": # Logi i blokada
                        try:
                            ...
                            if u_match:
                                ...
                                try:
                                    ...
                                    await u.send(f"⚠️ **ᴘᴀʀᴛɴᴇʀsᴛᴡᴏ ᴢᴀᴋᴏɴ́ᴄᴢᴏɴᴇ**\nᴛᴡᴏᴊᴇ ᴘᴀʀᴛɴᴇʀsᴛᴡᴏ ᴢᴏsᴛᴀᴌᴏ ᴜsᴜɴɪᴇ̨ᴛᴇ.\n**ᴘᴏᴡᴏ́ᴅ:** {powód}")
                                except: pass
                            await m_l.delete()
                        except: pass
                    del self.uzyte_serwery[srv_id]
                    await message.channel.send(f"✅ ᴜsᴜɴɪᴇ̨ᴛᴏ ɪ ᴢᴀʙʟᴏᴋᴏᴡᴀɴᴏ `{srv_id}`."); return
            ...
            if message.content.startswith("!potwierdz"):
                try:
                    ...
                    if u_id in self.zablokowani_uzytkownicy:
                        ...
                        await message.channel.send(f"✅ ᴏᴅʙʟᴏᴋᴏᴡᴀɴᴏ ᴜᴢʏᴛᴋᴏᴡɴɪᴋᴀ `{u_id}`.")
                except: pass
                return
